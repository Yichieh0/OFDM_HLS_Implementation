...
#pragma HLS INTERFACE mode=axis register_mode=both port=data_in register
	...
	ap_uint_64 read_in;
	ap_axiu_64 read_out;
	ap_uint_32 out_temp_r = 0;
	ap_uint_32 out_temp_i = 0;
	...
	ap_uint_64 para_in;
	ap_axiu_64 para_axiu;
	ap_axiu_64 data_axiu;
	data_axiu.data = 0;
	int module_id;
	int para_id;
	int para_val;
	...
	do{
		para_axiu = data_in.read();
		para_in = para_axiu.data;
		...
		if(module_id == module_num){
			...
			para_axiu.data = para_in;
			data_out.write(para_axiu);
		}
	...
	for(int k = 0; k<DATA_LEN*sym_num/(FFT_LEN-(FFT_LEN/pilot_width));k++){
		for(int t = 0; t < (2*FFT_LEN); t++){
			if(t < FFT_LEN){
				data_axiu = data_in.read();
				read_in = data_axiu.data;
			...
			if((t >= FFT_LEN)){ //64
				...
				data_axiu.data = (out_temp_r,out_temp_i);
				data_out.write(data_axiu);
			}
