	...
	for(int k = 0; k < DATA_LEN*sym_num/(FFT_LEN-(FFT_LEN/pilot_width)); k++){
		...
		for (int t = 0; t < (FFT_LEN/SSR); t++) {
			for(int i = 0; i < SSR; i++){
				...
				read_Tout0_in_r = (ap_fixed_bit)read_Tout0_in.real();
				read_Tout0_in_i = (ap_fixed_bit)read_Tout0_in.imag();
	...
	for(int k = 0; k < DATA_LEN*sym_num/(FFT_LEN-(FFT_LEN/pilot_width)); k++){
		...
		for (int t = 0; t < (FFT_LEN/SSR); t++) {
			for(int i = 0; i < SSR; i++){
				...
				read_Tout1_in_r = (ap_fixed_bit)read_Tout1_in.real();
				read_Tout1_in_i = (ap_fixed_bit)read_Tout1_in.imag();
