		...
		for(int i = 0; i < FFT_LEN+pilot_width; i++){
			...
			if(i >= pilot_width) {
				if(i_minus_D%pilot_width != 0) {
					...
					//out_r = ((last_data_re*in_h_re+last_data_im*in_h_im)/(in_h_re*in_h_re+in_h_im*in_h_im));
					//out_i = ((last_data_im*in_h_re-last_data_re*in_h_im)/(in_h_re*in_h_re+in_h_im*in_h_im));
