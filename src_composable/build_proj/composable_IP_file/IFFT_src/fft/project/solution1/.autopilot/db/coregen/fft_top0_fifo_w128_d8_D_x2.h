...
// ==============================================================
#ifndef fft_top0_fifo_w128_d8_D_x2_HH_
#define fft_top0_fifo_w128_d8_D_x2_HH_

#include <systemc>
using namespace std;
SC_MODULE(fft_top0_fifo_w128_d8_D_x2) {
    static const unsigned int DATA_WIDTH = 128;
    static const unsigned int ADDR_WIDTH = 4;
    static const unsigned int fft_top0_fifo_w128_d8_D_x2_depth = 9;
    ...
    sc_core::sc_signal< sc_dt::sc_lv<DATA_WIDTH> >  mStorage[fft_top0_fifo_w128_d8_D_x2_depth];
    ...
    SC_CTOR(fft_top0_fifo_w128_d8_D_x2) : mTrace(0) {
        ...
        SC_METHOD(proc_dout);
        sensitive << mOutPtr;
        for (unsigned i = 0; i < fft_top0_fifo_w128_d8_D_x2_depth; i++) {
        ...
        if (reset.read() == sc_dt::SC_LOGIC_1) {
            ...
            if (if_read_ce.read() == sc_dt::SC_LOGIC_1
               ...
               && internal_empty_n.read() == sc_dt::SC_LOGIC_1) {
                ...
                if (mOutPtr.read().to_uint() == (fft_top0_fifo_w128_d8_D_x2_depth-1)) {
                    ptr = 0;
                ...
                assert(ptr.to_uint() < fft_top0_fifo_w128_d8_D_x2_depth);
                mOutPtr.write(ptr);
            }
            if (if_write_ce.read() == sc_dt::SC_LOGIC_1
                ...
                if (ptr.to_uint() == (fft_top0_fifo_w128_d8_D_x2_depth-1)) {
                    ptr = 0;
                    mFlag_nEF_hint.write(~mFlag_nEF_hint.read());
                } else {
                    ptr++;
    ...
    void proc_dout() {
        ...
        if (ptr.to_uint() > fft_top0_fifo_w128_d8_D_x2_depth) {
...
#endif //fft_top0_fifo_w128_d8_D_x2_HH_
