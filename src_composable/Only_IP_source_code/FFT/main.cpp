...
int main(int argc, char *argv[])
{
	...
	for (int k = 0; k < DATA_LEN*sym_num*FFT_LEN/(FFT_LEN-(FFT_LEN/pilot_width)); k++){
		in_r = rand()/(RAND_MAX+1.0)*8-4; ///(RAND_MAX+1.0)
		in_i = rand()/(RAND_MAX+1.0)*8-4; ///(RAND_MAX+1.0)
		...
		cout << k << " : in_r = " << in_r << " in_i = " << in_i << endl;
	...
	for (int k = 0; k < DATA_LEN*sym_num*FFT_LEN/(FFT_LEN-(FFT_LEN/pilot_width)); k++){
		...
		cout << k << " : out_r = " << out_r_temp<< " out_i = " << out_i_temp << endl;
